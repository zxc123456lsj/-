...
class RNNClassifier(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim):
        ...
        self.embedding = nn.Embedding(vocab_size, embedding_dim)  # 嵌入层不变
        # 替换LSTM为RNN，参数保持一致（batch_first=True）
        self.rnn = nn.RNN(embedding_dim, hidden_dim, batch_first=True)
        self.fc = nn.Linear(hidden_dim, output_dim)  # 全连接层不变

    def forward(self, x):
        # 嵌入层：batch_size * seq_len -> batch_size * seq_len * embedding_dim
        embedded = self.embedding(x)

        # RNN前向传播：LSTM返回(out, (hidden, cell))，RNN仅返回(out, hidden)
        rnn_out, hidden_state = self.rnn(embedded)

        # 取最后一层的隐藏状态做分类（和LSTM逻辑一致）
        out = self.fc(hidden_state.squeeze(0))
        return out
...
class GRUClassifier(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim):
        ...
        self.embedding = nn.Embedding(vocab_size, embedding_dim)  # 嵌入层不变
        # 替换LSTM为GRU，参数保持一致（batch_first=True）
    ...
    def forward(self, x):
        ...
        # GRU前向传播：LSTM返回(out, (hidden, cell))，RNN仅返回(out, hidden)
        rnn_out, hidden_state = self.rnn(embedded)
