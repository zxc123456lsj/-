...
class SimpleClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.fc2(out)
        return out
class SimpleClassifierDeeper(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim): # 层的个数 和 验证集精度
        # 层初始化
        super(SimpleClassifier, self).__init__()
        self.fc1 = nn.Linear(input_dim, hidden_dim)
        self.relu = nn.ReLU()
        self.fc2 = nn.Linear(hidden_dim, hidden_dim // 2)  # 新增一层
...
model2 = SimpleClassifier(vocab_size, hidden_dim*2, output_dim) # 维度和精度有什么关系？
...
index2 = [i for i in range(len(loss_list2))]
plt.plot(index, loss_list, label=f'loss', color='red', linewidth=0.5)
plt.plot(index2, loss_list2, label=f'loss', color='blue', linewidth=0.5)
plt.xlabel('iter')
plt.ylabel('loss')
plt.legend()
plt.grid(True)
