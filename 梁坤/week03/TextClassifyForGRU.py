...
def choose_model(model_type, vocab_size, embedding_dim, hidden_dim, output_dim):
    if model_type == "LSTM":
        return LSTMClassifier(vocab_size, embedding_dim, hidden_dim, output_dim)
    elif model_type == "GRU":
        return GRUClassifier(vocab_size, embedding_dim, hidden_dim, output_dim)
    elif model_type == "RNN":
        return RNNClassifier(vocab_size, embedding_dim, hidden_dim, output_dim)
    else:
...
# 对于短序列输入，GRU的收敛速度更快，准确率最高
# LSTM，需要多训练几个轮次，准确率才能提高，收敛速度较慢
# RNN，准确率很低，结果不稳定

# model = choose_model("RNN", vocab_size, embedding_dim, hidden_dim, output_dim)
# model = choose_model("LSTM", vocab_size, embedding_dim, hidden_dim, output_dim)
model = choose_model("GRU", vocab_size, embedding_dim, hidden_dim, output_dim)
